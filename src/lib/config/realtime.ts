		...
		return {
			...
			successRate:
				this.metrics.totalEvents > 0
					? ((this.metrics.totalEvents - this.metrics.failedEvents) / this.metrics.totalEvents) *
						100
					: 100
		};
	}
 ...
 */
export function validateBattleConfig(config: Partial<BattleConfig>): {
	valid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
	...
	if (
		config.case_markup_percentage !== undefined &&
	...
	if (
		config.mode_rake_percentage !== undefined &&
...
export function getAnimationDuration(
	eventType: 'round_start' | 'pull_reveal' | 'round_complete' | 'battle_complete'
): number {
	const eventTypeMap = {
		round_start: 'ROUND_START',
		pull_reveal: 'PULL_REVEAL',
		round_complete: 'ROUND_COMPLETE',
		battle_complete: 'BATTLE_COMPLETE'
	} as const;

	const configKey = eventTypeMap[eventType];
	return REALTIME_CONFIG.ANIMATION_DURATION[configKey];
}

/**
 * Validate battle event data for realtime events
 */
export function validateBattleEventData(
	eventType: string,
	data: any
