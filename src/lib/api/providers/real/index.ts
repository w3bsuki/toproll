	...
	async getProfile(): Promise<UserProfile | null> {
		try {
			...
			return {
				...
				last_seen: data.profile.last_seen,
				created_at: data.profile.created_at,
				updated_at: data.profile.updated_at,
				total_wagered: Number(data.profile.total_wagered) || 0,
				total_profit: Number(data.profile.total_profit) || 0,
				win_rate: Number(data.profile.win_rate) || 0,
				biggest_win: Number(data.profile.biggest_win) || 0,
				case_battle_wins: Number(data.profile.case_battle_wins) || 0
			};
		} catch {
			return null;
		}
