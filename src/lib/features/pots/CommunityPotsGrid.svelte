<script lang="ts">
	...
	import CommunityPotCard from './CommunityPotCard.svelte';
		...
		filter === 'all'
			...
			: pots.filter((pot) => {
					switch (filter) {
						...
						case 'rain':
							return pot.name.toLowerCase().includes('rain');
						case 'vip':
							return pot.name.toLowerCase().includes('vip');
						case 'flash':
							return pot.name.toLowerCase().includes('flash');
						case 'ending-soon':
							return pot.status === 'locked';
						default:
							return true;
					}
				})
...
<section class={`space-y-6 ${className}`}>
	...
	<div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
		<div class="space-y-2">
			...
			<p class="text-muted-foreground text-base">Join community rewards and rain events</p>
		</div>
	...
	<div class="flex flex-wrap gap-2">
		{#each filters as filterOption}
			...
			>
				...
				{filterOption.label}
				{#if filterOption.id === 'all'}
					<span class="bg-primary/20 text-primary rounded-full px-2 py-0.5 text-xs">
						{pots.length}
					</span>
				{:else}
					<span class="bg-primary/20 text-primary rounded-full px-2 py-0.5 text-xs">
	...
	{:else if error}
		<div
			class="bg-surface/30 border-border/40 flex flex-col items-center gap-4 rounded-3xl border py-12 text-center"
		>
	...
	{:else if displayPots.length === 0}
		...
		>
			...
			{#if filter !== 'all'}
				<Button variant="outline" onclick={() => handleFilterChange('all')}>Show All Pots</Button>
			{/if}
		</div>
	{:else}
		<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
			{#each displayPots as pot (pot.id)}
				<CommunityPotCard {pot} onJoin={onJoinPot} onView={onViewPot} />
			{/each}
		</div>
