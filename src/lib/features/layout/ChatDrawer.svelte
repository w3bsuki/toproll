<script lang="ts">
	// ✅ NEW: Use modern Svelte 5 state management
	...
	// Keep homepage store subscriptions (will migrate later)
	$effect(() => {
...
<Sheet.Root open={ui.chatOpen} onOpenChange={(open) => (!open ? ui.closeChat() : undefined)}>
	<Sheet.Content
		side="bottom"
