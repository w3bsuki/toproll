<script lang="ts">
	...
	import type { Case, CreateBattleRequest } from '$lib/types/index';
								...
								<CardContent>
									<div class="max-h-96 space-y-3 overflow-y-auto">
										{#each availableCases as case_ (case_.id)}
											<button
												type="button"
												class="border-border/40 hover:border-primary/50 w-full cursor-pointer rounded-lg border p-3 text-left transition-all {selectedCases.some(
													(c) => c.id === case_.id
												)
