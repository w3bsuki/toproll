...
export class BattleEventBroadcaster {
   ...
   */
  async broadcastEvent(battleId: string, event: Omit<BattleEvent, 'battle_id' | 'timestamp'>): Promise<boolean> {
    try {
      ...
      console.error(`Failed to broadcast battle event ${event.type} for battle ${battleId}:`, error);
      return false;
    }
  }
   ...
   */
  async broadcastParticipantJoined(battleId: string, participant: BattleParticipant): Promise<boolean> {
   ...
   */
  async broadcastRoundStart(battleId: string, roundIndex: number, caseId: string): Promise<boolean> {
    return this.broadcastEvent(battleId, {
      type: 'round_start',
      data: {
...
export class BattleOrchestratorRealtime {
   ...
   */
  async onParticipantJoined(battleId: string, participant: BattleParticipant): Promise<void> {
    await this.broadcaster.broadcastParticipantJoined(battleId, participant);
  }

  /**
   * Called when battle is locked and starting
   */
  async onBattleLocked(battleId: string, participantIds: string[]): Promise<void> {
    await this.broadcaster.broadcastBattleLocked(battleId, participantIds);
  }

  /**
   * Called when a new round starts
   */
  async onRoundStart(battleId: string, roundIndex: number, caseId: string): Promise<void> {
    await this.broadcaster.broadcastRoundStart(battleId, roundIndex, caseId);
  }
  ...
  async onRoundPull(
    ...
    await this.broadcaster.broadcastRoundPull(battleId, participantId, item, hash, nonce, clientSeed);
  }
  ...
  async onRoundResult(
    ...
    await this.broadcaster.broadcastRoundResult(battleId, roundIndex, pulls, subtotals);
  }
  ...
  async onBattleSettled(
    ...
    await this.broadcaster.broadcastBattleSettled(battleId, winnerId, winners, totals, tieBreak);
  }

  /**
   * Called when battle is cancelled
   */
  async onBattleCancelled(battleId: string, reason: string): Promise<void> {
    await this.broadcaster.broadcastBattleCancelled(battleId, reason);
  }
}
...
export type { BattleEventBroadcaster, BattleOrchestratorRealtime };