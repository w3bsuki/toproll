...
export class BattleOrchestrator extends EventEmitter {
	...
	private setupRealtimeIntegration() {
		...
		this.on('battle_locked', async (payload) => {
			await this.broadcastWithRateLimit('battle_locked', payload.battleId, async () => {
				const participantIds = payload.participants.map((p: BattleParticipant) => p.id);
		...
		this.on('round_result', async (payload) => {
			await this.broadcastWithRateLimit('round_result', payload.battleId, async () => {
				const pulls = payload.pulls.map((pull: BattlePull) => ({
				...
				return this.realtime.onRoundResult(
					payload.battleId,
					payload.index,
					pulls,
					payload.subtotals
	...
	async joinBattle(
		...
		userId: string,
		clientSeed?: string
	): Promise<{ participant?: BattleParticipant; error?: string }> {
		try {
	...
	async startBattle(battleId: string) {
		try {
			...
			await this.supabase.from('battles').update({ status: 'in_progress' }).eq('id', battleId);
			...
			await this.supabase.from('battles').update({ status: 'cancelled' }).eq('id', battleId);
		}
	}
			...
			for (const participant of participants) {
				...
				const { data: pull, error: pullError } = await this.supabase
					...
					.insert({
						...
						client_seed: rollResult.clientSeed,
	...
	private async settleBattle(battleId: string, participants: BattleParticipant[]) {
		try {
			// Get battle data to determine current round and mode
			...
			const roundIndex = battle.current_round;
        ...
        `
			...
			if (tiedParticipants.length > 1) {
				...
				if (serverSeed && tiedParticipants.length > 0) {
					...
					const tieBreakHash = require('crypto')
						.createHmac('sha256', serverSeed)
						.update(tieBreakSeed)
						.digest('hex');

					const tieBreakRoll =
						parseFloat('0x' + tieBreakHash.substring(0, 8)) / parseFloat('0x10000000000000000');
			...
			const { error: settleError } = await this.supabase.rpc('settle_battle_transaction', {
				p_battle_id: battleId,
				p_winner_participant_ids:
					tiedParticipants.length > 0 ? tiedParticipants.map((p) => p.id) : [winnerId],
